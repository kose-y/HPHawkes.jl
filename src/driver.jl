...
function loglik2(d::HawkesStorage{T},
    sigmaXprec::T, tauXprec::T, tauTprec::T, omega::T, theta::T, mu0::T, dimX;
    first_idx = 0,
    last_idx = d.m
    ) where T
    d.queue(d.k_loglik2, (last_idx - first_idx) * TPB, TPB, 
        d.locations_buff, d.times_buff, d.popdensity_buff, 
        d.likContribs_buff, 
        sigmaXprec, tauXprec, tauTprec, omega, theta, mu0, 
        Int32(dimX), UInt32(first_idx), UInt32(last_idx), UInt32(d.m)) |> cl.wait
    cl.copy!(d.queue, d.likContribs, d.likContribs_buff) |> cl.wait
...
function loglik1_cb(d::HawkesStorage{T},
    sigmaXprec::T, tauXprec::T, tauTprec::T, omega::T, theta::T, mu0::T, dimX;
    first_idx = 0,
    last_idx = d.m
    ) where T
    d.queue(d.k_loglik1_cb, (last_idx - first_idx) * TPB, TPB, d.locations_buff, d.times_buff, d.likContribs_buff, 
        sigmaXprec, tauXprec, tauTprec, omega, theta, mu0, 
        Int32(dimX), UInt32(first_idx), UInt32(last_idx), UInt32(d.m)) |> cl.wait
    cl.copy!(d.queue, d.likContribs, d.likContribs_buff) |> cl.wait
    d.queue(d.k_sum, TPB, TPB, d.likContribs_buff, d.likContribs_reduced_buff, UInt32((last_idx - first_idx) ÷ 8)) |> cl.wait# what is done if m not multiple of 8?
...
function loglik2_cb(d::HawkesStorage{T},
    sigmaXprec::T, tauXprec::T, tauTprec::T, omega::T, theta::T, mu0::T, dimX;
    first_idx = 0,
    last_idx = d.m
    ) where T
    d.queue(d.k_loglik2_cb, (last_idx - first_idx) * TPB, TPB, 
        d.locations_buff, d.times_buff, d.popdensity_buff, 
        d.likContribs_buff, 
        sigmaXprec, tauXprec, tauTprec, omega, theta, mu0, 
        Int32(dimX), UInt32(first_idx), UInt32(last_idx), UInt32(d.m)) |> cl.wait
